...
export async function createCommission(dealId, amount) {
    const response = await apiClient.post(`/admin/deals/${dealId}/commission`, { amount });
    return response.data;
}
export async function updateCommissionStatus(commissionId, status) {
    const response = await apiClient.patch(`/admin/commissions/${commissionId}/status`, { status });
    return response.data;
}
